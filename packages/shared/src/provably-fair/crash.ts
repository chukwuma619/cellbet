...
/** Provably-fair crash point from server seed + round key (uniform → multiplier). */
export function computeCrashMultiplier(serverSeed: string, roundKey: string): number {
  const h = sha256ConcatUtf8([serverSeed, roundKey]);
  const u =
...
export function computeRunningDurationMs(serverSeed: string, roundKey: string): number {
  const h = sha256ConcatUtf8([serverSeed, roundKey, "duration"]);
  const u32 =
 ...
 * `crash_multiplier === computeCrashMultiplier(server_seed, round_key)`.
 */
